...
import { executeHackInstruction } from './simulatorCpu';
...
useEffect(() => {
  ...
  simulationRef.current = setInterval(() => {
    if (pc < rom.length && rom[pc] !== undefined) {
      const nextState = executeHackInstruction({
        instruction: rom[pc],
        aRegister,
        dRegister,
        pc,
      ...
      setARegister(nextState.aRegister);
      setDRegister(nextState.dRegister);
      setPc(nextState.pc);
      if (nextState.memory !== memory) {
